...
async function convertFile(fileState) {
        ...
        options: {
            onProgress: ({transferredBytes , totalBytes }) => {
                if (totalBytes) {
                    setFile({
                        ...file,
                        progress: Math.round((transferredBytes  / totalBytes) * 50)
                    })
                }
            }
        }
...
async function postToLambda(file, signedUrl) {
    const url = 'https://mrslu4s2s2asuo4in7lp54rgou0pyqnx.lambda-url.ca-central-1.on.aws/';
    const data = {
        file: signedUrl,
